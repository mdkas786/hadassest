  ...
  LOOP
    ...
    UPDATE public.investments
       SET amount_received = COALESCE(amount_received,0) + v_amt,
           status = CASE WHEN COALESCE(amount_received,0) + v_amt
                          >= COALESCE(expected_2x, amount_invested * 2)
    ...
    -- LEVEL INCOME (single-level, 10% of ROI) for qualifying sponsor
      ...
      IF v_directs >= 2 THEN
        ...
        IF v_sp_inv.id IS NOT NULL THEN
          v_sp_remaining := GREATEST(0,
            COALESCE(v_sp_inv.expected_2x, v_sp_inv.amount_invested * 2)
            - COALESCE(v_sp_inv.amount_received,0));
          ...
          IF v_sp_inv.id IS NOT NULL THEN
            UPDATE public.investments
               SET amount_received = COALESCE(amount_received,0) + v_level_amt,
                   ...
                   status = CASE WHEN COALESCE(amount_received,0) + v_level_amt
                                  >= COALESCE(expected_2x, amount_invested * 2)
                                 THEN 'completed'::investment_status ELSE status END
